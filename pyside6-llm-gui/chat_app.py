    ...
    def render(self, text):
        ...
        try:
            ...
            def replace_block(match):
                ...
                # Extract raw code by stripping HTML tags for the copy function
                # This is a heuristic; technically unescaping HTML entities is also needed
                raw_code = re.sub(r'<[^>]+>', '', content)
                # Unescape common HTML entities
                raw_code = raw_code.replace('&lt;', '<').replace('&gt;', '>').replace('&amp;', '&').replace('&quot;', '"').replace('&#39;', "'")
                code_blocks.append(raw_code)
                
                return (
                    f'<table class="highlight" width="100%" bgcolor="#2E3440" border="0" cellpadding="0" cellspacing="0" style="border-radius: 6px; margin: 10px 0; border: 1px solid {NORD["nord3"]};">'
                    f'<tr><td align="right" style="background-color: #191c21; padding: 4px 8px; border-bottom: 1px solid {NORD["nord3"]}; border-top-left-radius: 6px; border-top-right-radius: 6px;">'
                    f'<a href="copy_code:{index}" style="color: {NORD["nord8"]}; text-decoration: none; font-weight: bold; font-family: sans-serif; font-size: 11px;">📋 Copy</a>'
                    f'</td></tr>'
                    f'<tr><td style="padding: 12px;">{content}</td></tr></table>'
                )
            ...
            {html}
            """
            return styled_html, code_blocks
...
class MessageWidget(QFrame):
    ...
    def handle_link_click(self, url):
        ...
        # Robustly check for copy_code schema, handling potential Qt URL normalization
        # pattern matches copy_code:N where N is the index
        match = re.search(r'copy_code[:%3A]+(\d+)', url_str)
        ...
        if match:
            try:
                ...
                if 0 <= index < len(self.code_blocks):
                    ...
                    self.content_display.setToolTip("Copied!")
                    QTimer.singleShot(1000, lambda: self.content_display.setToolTip(""))
    ...
    def adjust_height(self):
        ...
        if self.thinking_frame and self.thinking_expanded:
            ...
            thinking_height = 30 # Approx closed height
            
        # Header + content + thinking + padding
        total_height = doc_height + thinking_height + 80
...
class InputArea(QWidget):
    ...
    def setup_ui(self):
        ...
        self.stop_btn.setFixedSize(60, 36)
